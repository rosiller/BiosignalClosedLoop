...
class EEG:
    ...
    def __init__(
        ...
        self.other = other
        self.backend = self._get_backend(self.device_name) # determine if board uses brainflow or muselsl backend
        self.initialize_backend()
        self.streaming_active=False
        self.session_list = []
        
    def _get_backend(self, device_name):
        if device_name in EEG.brainflow_devices:
            return "brainflow"
    ...
    def start(self, fn='', duration=None, eeg=True, ppg=False, acc=False,gyro=False):
        ...
        if self.backend == "brainflow":  # Start brainflow backend
            ...
            self._start_muse(duration,eeg,ppg, acc, gyro)
    ...
    def get_recent(self,max_samples=100):
        ...
        if self.backend == "brainflow":
            df = self._brainflow_get_recent(max_samples)
        elif self.backend == "muselsl":
    ...
    def _start_muse(self, duration, eeg, ppg, acc, gyro):
        ...
        if sys.platform in ["linux", "linux2", "darwin"]:
            ...
            self.stream_process = Process(
                target=stream, args=(self.muses[0]["address"], ), kwargs={'eeg_disabled':not eeg,'ppg_enabled':ppg, 'acc_enabled': acc, 'gyro_enabled': gyro})
    ...
    def _muse_collect_data(self,rec_time=10):
        # self.session = RecordingSession(data_sources=['eeg'])
        # self.eeg = []
        # self.eeg_timestamp = []
        self._muse_recent_inlet = self.get_inlet('EEG')
        self.f_samp, self.labels,self.nb_channels = self.extract_info()
        print(self.f_samp,self.labels,self.nb_channels)
        eeg_max_samples = int(self.sfreq)

        start_time = time.time()            
        while time.time()-start_time<rec_time:
            
            try:
                samples_eeg = np.array(self._muse_recent_inlet.pull_chunk(timeout = 1.0, max_samples = eeg_max_samples)[0])[:,:-1]  # Remove the right aux

                # data, timestamp = self._muse_recent_inlet.pull_chunk(timeout=1.0, max_samples=mlsl_cnsts.LSL_EEG_CHUNK)
                print(samples_eeg)
    ...
    def get_inlet(self,sensor:str='EEG')->None:
    
        streams = resolve_byprop('type', sensor, timeout=mlsl_cnsts.LSL_SCAN_TIMEOUT)
        
        # Troubleshooting (when streams is empty):
        if streams==[]:
            print('Connect to a network') # Needs this
        ...
        if sensor=='EEG':
            ...
            max_chunklen = mlsl_cnsts.LSL_PPG_CHUNK

        # inlet = StreamInlet(streams[0], max_chunklen=max_chunklen)
        _muse_inlet = StreamInlet(streams[0], max_chunklen=max_chunklen)
        
        return _muse_inlet
    
    def extract_info(self)->tuple:
        info = self._muse_recent_inlet.info()
        if info:
            description = info.desc()
            ch = description.child('channels').first_child()
            ch_names = [ch.child_value('label')]
            n_chan = info.channel_count()-1 # Not using the Right aux

            for i in range(n_chan):
                        ch = ch.next_sibling()
                        ch_names.append(ch.child_value('label'))
            labels = ch_names[:-1] # Removing the last unused value
            f_samp = info.nominal_srate()
        # else:
        #     f_samp = 256
        #     labels = ['TP9','AF7','AF8','TP10']
        #     n_chan = len(labels)

        return f_samp, labels, n_chan
    
    def _stop_muse(self):
        if self.stream_process.is_alive():
            print('Terminating it manually')
            os.kill(self.stream_process.pid, signal.SIGINT)  # replace pid with your process id
